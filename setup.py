...
# Ensure modern libstdc++ features are enabled and document ABI preference
aimrocks_extra_compile_args += ["-D_GLIBCXX_USE_CXX11_ABI=1"]
...
local_include_dir = os.path.abspath(os.path.join(os.path.dirname(__file__), "src/aimrocks/include"))
local_lib_dir = os.path.abspath(os.path.join(os.path.dirname(__file__), "src/aimrocks"))
...
if os.environ.get("AIMROCKS_EMBED_ROCKSDB", "1") == "1":
    ...
    for source in third_party_headers:
        ...
        copy_tree(source, destination, preserve_symlinks=False, update=True)
